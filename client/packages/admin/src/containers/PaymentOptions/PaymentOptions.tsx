...
import {
  ...
  Image,
  ImageWrapper,
} from './PaymentOptions.style';
import NoResult from '../../components/NoResult/NoResult';
import {Plus} from "../../components/AllSvgIcon";
import {useDrawerDispatch} from "../../context/DrawerContext";
import ActionWrapper from "../PaymentOptionForm/ActionWrapper";
...
export default function Coupons() {
  ...
  const openDrawer = useCallback(
      () => dispatch({ type: 'OPEN_DRAWER', drawerComponent: 'PAYMENT_OPTION_FORM' }),
      [dispatch]
  );
  ...
  function handlePreviousDisabled(data) {
    const result = (data ? data.paymentOptions.totalCount === 0 : false) || offset === 0;
    return result;
  }
  function handleNext() {
    setOffset(offset+12);
  ...
  function handleNextDisabled(data) {
    const result = data ? !data.paymentOptions.hasMore : true;
    return result;
  }
        ...
        <Row>
          <Col md={12}>
            ...
            <Wrapper style={{ boxShadow: '0 0 5px rgba(0, 0 , 0, 0.05)' }}>
              <TableWrapper>
                <StyledTable $gridTemplateColumns="minmax(70px, 70px) minmax(150px, auto) minmax(150px, auto) minmax(70px, 70px) minmax(120px, auto) minmax(120px, auto) minmax(160px, 160px)">
                  ...
                  {data ? (
                      data.paymentOptions.items.length ? (
                          data.paymentOptions.items.map((item: any, index: number) => {
                                return (
