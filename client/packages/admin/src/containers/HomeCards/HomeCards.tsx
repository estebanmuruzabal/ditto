...
import ActionWrapper from "../HomeCardForm/ActionWrapper";
...
export default function HomeCards() {
  ...
  const openDrawer = useCallback(
      () => dispatch({ type: 'OPEN_DRAWER', drawerComponent: 'HOMECARD_FORM' }),
      [dispatch]
  );
  ...
  function handlePreviousDisabled(data) {
    const result = (data ? data.homeCards.totalCount === 0 : false) || offset === 0;
    return result;
  }
  function handleNext() {
    setOffset(offset+12);
  ...
  function handleNextDisabled(data) {
    const result = data ? !data.homeCards.hasMore : true;
    return result;
  }
            ...
            <Wrapper style={{ boxShadow: '0 0 5px rgba(0, 0 , 0, 0.05)' }}>
              <TableWrapper>
                <StyledTable $gridTemplateColumns="minmax(70px, 70px) minmax(150px, auto) minmax(150px, auto) minmax(70px, 70px) minmax(70px, 70px) minmax(120px, auto) minmax(160px, 160px)">
                  ...
                  {data ? (
                      data.homeCards.items.length ? (
                          data.homeCards.items.map((item: any, index: number) => {
                                return (
                                    <React.Fragment key={index}>
                                      <StyledBodyCell>{index+1}</StyledBodyCell>
                                      <StyledBodyCell>{item.name}</StyledBodyCell>
                                      <StyledBodyCell>{item.url}</StyledBodyCell>
                                      ...
                                      <StyledBodyCell>{item.status ? 'Active' : 'Inactive'}</StyledBodyCell>
