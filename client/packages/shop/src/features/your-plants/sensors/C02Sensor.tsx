...
import { SensorsTypes, C02SensorMode, WeekDays, fourRelaysOptions, manualModeOptions, c02SensorModeOptions } from 'utils/constant';
...
import C02Reading from './sensor-readings/C02Reading';
...
const C02Sensor: React.FC<Props> = ({ errorId, plant, settingType, handleSettingsChange, onDeleteSchedule, data, openTab, setOpenTab, handleDeleteSensor }) => {
    ...
    const selectedMode = c02SensorModeOptions.find((option) => option.value === setting.mode);
    ...
    return (
        <PlantsSensorContainer style={{ height: tabIsOpen ? '100%' : '66px' }} onClick={(e) => { e.stopPropagation(); setOpenTab(tabIsOpen ? '' : settingType); }}>
            ...
            { (setting?.mode === C02SensorMode.NONE && !!openTab) && (
                <ListItem>
          ...
          <ListItem style={{ justifyContent: 'flex-start' }}>
            <ListTitle>
              <Text>
                <FormattedMessage
                  id="c02Id"
                  defaultMessage="c02Id"
                />
              </Text>
            </ListTitle>
            <ListDes>
                <C02Reading module={module} plantId={plant.plantId} latestDataFetched={data} />
            </ListDes>
          </ListItem>
          ...
          { (setting?.mode !== C02SensorMode.NONE) && (
            <ListItem style={{ justifyContent: 'flex-start' }}>
                <ListTitle>
                <Text>
                    <FormattedMessage
          ...
          { (setting?.mode === C02SensorMode.MANUAL || setting?.mode === C02SensorMode.SCHEDULE) && (
            <ListItem>
              <ListTitle>
                <Text>
                  <FormattedMessage
          ...
          { ((setting.mode === C02SensorMode.SCHEDULE) && hasRelayAsociated) && (
            <>
                ...
                <Button
                    ...
                    onClick={() => handleModal(
                        AddTimeSchedule,  { name: 'add-co2-1-schedule', plant, id: data?.getUser?.id, settingType } )}
                >
            ...
            { setting.mode === C02SensorMode.WHEN_MIN_ACTION_AUTOMATED && (
                <>
                    <ListItem>
                        <ListTitle>
                            <Text>
